...
# Adapted from https://github.com/fengzhanglab/Screening_Protocols_manuscript.git 
import gzip
...
def count_spacers(input_file, fastq_file, output_file, guide_g): 
	...
	# open fastq files
	fileNames = fastq_file.split(',')
	for fileName in fileNames:
		print('Opening file ' + str(fileName))
		try:
			if str(fileName).endswith('.fastq'):
	...
	percent_matched = round(perfect_matches/float(perfect_matches + non_perfect_matches) * 100, 3)
	...
	percent_no_reads = round(guides_no_reads/float(len(dictionary.values())) * 100, 3)
	...
	if top_10 != 0 and bottom_10 != 0:
		skew_ratio = round(top_10/bottom_10,5)
	else:
